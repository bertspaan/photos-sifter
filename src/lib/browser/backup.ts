...
const pathSchema = z.string().refine(safeRelativePath, 'Invalid relative photo path.');
...
export async function exportWorkspace(database: ReviewDatabase = db) {
	return database.transaction('r', database.tables, async () => ({
		...
		folders: (await database.folders.toArray()).map(({ handle, ...metadata }) => metadata),
		...
		deletions: (await database.deletions.toArray()).filter((d) => d.state !== 'pending')
	}));
}
export async function restoreWorkspace(input: unknown, database: ReviewDatabase = db) {
	...
	for (const p of data.photos) {
		...
		if (
			p.source === 'local' &&
			(!p.folderId || !folders.has(p.folderId) || !safeRelativePath(p.localPath))
		)
			throw new Error('Backup has an invalid folder reference.');
	}
	for (const f of data.files)
		if (!folders.has(f.folderId)) throw new Error('Backup has an invalid folder reference.');
	...
	for (const row of [...data.history, ...data.deletions])
		if (!photos.has(row.photoId)) throw new Error('Backup has an invalid photo reference.');
	...
	await database.transaction('rw', database.tables, async () => {
		...
		await database.settings.put({ key: 'activeFolder', value: data.activeFolder });
	});
