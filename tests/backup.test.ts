...
it('does not export folder capabilities or confirmation tokens', async () => {
	...
	await source.deletions.put({ batch: 'secret', photoId: 'a', state: 'pending', error: '' });
...
it('keeps interrupted deletions locked after restoring', async () => {
	await source.deletions.put({ batch: 'b', photoId: 'a', state: 'executing', error: '' });
	await restoreWorkspace(await exportWorkspace(source), target);
	expect((await target.deletions.get(['b', 'a']))?.state).toBe('uncertain');
	await expect(
		new ReviewStore(target).command('decision', { id: 'a', decision: 'keep', revision: 0 })
	).rejects.toThrow('Verify');
});
it('refuses to overwrite an existing workspace', async () => {
	await target.photos.put({ ...blankPhoto('existing'), filename: 'existing.jpg' });
	await expect(restoreWorkspace(await exportWorkspace(source), target)).rejects.toThrow(
		'empty workspace'
	);
...
it('rejects unsafe preview URLs before restoring anything', async () => {
	...
	await expect(restoreWorkspace(backup, target)).rejects.toThrow('Unexpected image host');
