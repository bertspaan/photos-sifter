<script lang="ts">
	...
	import { downloadJson, exportWorkspace, restoreWorkspace } from '$lib/browser/backup';
	...
	let appState = $state<AppState>({ imports: [], localCount: 0, root: '', undoCount: 0 });
	...
	let queue = $derived(
		photos.filter((p) => !p.trashed && (selectedView === 'all' || p.decision === selectedView))
	);
	let current = $derived(queue[index]);
	let counts = $derived({
		all: photos.filter((p) => !p.trashed).length,
		unreviewed: photos.filter((p) => !p.trashed && p.decision === 'unreviewed').length,
	...
	let marked = $derived(
		photos.filter(
			(p) => !p.trashed && p.decision === 'delete' && p.source === 'google' && p.isOwned
		)
	);
	let selectedAccount = $derived(accounts.find((a) => String(a.tabId) === tabValue));
	...
	async function loadPhotos() {
		...
		if (useList && !names.length)
			throw new Error('Enter at least one filename, or turn the list filter off.');
	...
	async function connect() {
		const result = await bridge<{ tabs: { tabId: number; account: string }[] }>('ping');
		accounts = result.tabs;
		if (!accounts.length)
			throw new Error('Open Google Photos in Chrome and sign in, then connect again.');
		if (!accounts.some((a) => String(a.tabId) === tabValue)) tabValue = String(accounts[0].tabId);
		connection = 'Connected';
	}
	function connectionPayload() {
		if (!selectedAccount) throw new Error('Connect to a Google Photos tab first.');
	...
	async function chooseFolder(reattachId?: string) {
		if (!supportsFolderPicker())
			throw new Error('Open this app in Chrome or Edge to choose a local folder.');
		let handle: FileSystemDirectoryHandle;
		try {
			handle = await window.showDirectoryPicker({ id: 'whatsapp-backup', mode: 'read' });
		} catch (e) {
		...
		try {
			...
			notice = fmt(appState.localCount) + ' local photos indexed in this browser.';
		} catch (e) {
	...
	async function reconnectFolder(selected: FolderRecord) {
		...
		if (permission !== 'granted')
			throw new Error('Folder access was not granted. Your saved decisions are unchanged.');
	...
	async function importWorkspace(event: Event) {
		...
		if (!file) return;
		await run(async () => {
			if (file.size > 200_000_000) throw new Error('Workspace file is too large.');
			...
			index = 0;
			await loadPhotos();
			notice = 'Workspace restored. Reconnect local folders before opening local photos.';
		});
		input.value = '';
	}
	...
	async function startReview() {
		...
		if (matchOnly && !appState.localCount)
			throw new Error('Choose your backup folder first, or turn off filename matching.');
	...
	async function importMore() {
		...
		if (!selected || selected.source !== 'google' || selected.status === 'complete') return;
		const conn = connectionPayload();
		...
		try {
			while (!stopRequested) {
				const page = await bridge<any>('page', { ...conn, query: selected.query, cursor });
	...
	async function decide(decision: Decision) {
		if (!current || busy || deleting || setupOpen || deleteOpen || helpOpen) return;
		...
		const p = current,
			oldIndex = index;
		await run(async () => {
			const result = await review('decision', { id: p.id, decision, revision: p.revision });
			photos = photos.map((x) => (x.id === p.id ? { ...x, ...result.photo } : x));
	...
	function handleKey(e: KeyboardEvent) {
		...
		if (['k', 'y', 'arrowright', 'd', 'n', 'arrowleft', ' ', 'z'].includes(key)) e.preventDefault();
	...
	async function prepareDelete() {
		...
		await run(async () => {
			preview = await review('delete-preview', { ids: marked.slice(0, 100).map((p) => p.id) });
			failedImages = [];
			loadedImages = [];
			deleteOpen = true;
		});
	...
	async function confirmDelete() {
		...
		try {
			...
			if (conn.account !== snapshot.photos[0].account)
				throw new Error('Reconnect to the Google account shown in this preview.');
			const confirmed = await review('delete-confirm', { token: snapshot.token });
			...
			for (const p of confirmed.photos as Photo[]) {
				...
				try {
					...
					if (!result.verified) throw new Error('Deletion could not be verified.');
	...
	async function refreshPreview() {
		await run(async () => {
			...
			if (conn.account !== p.account) throw new Error('Connect to this photo’s Google account.');
			const photo = await bridge<any>('refresh', { ...conn, mediaKey: p.mediaKey });
			const result = await review('photo-refresh', { id: p.id, account: photo.account, photo });
	...
	onMount(() => {
		...
		try {
			const p = JSON.parse(localStorage.getItem('photo-review-preferences') || '{}');
			importId = p.importId || '';
				...
				context.registerTool(
					{
						...
						execute: () => ({
							counts,
							current: current ? { filename: current.filename, decision: current.decision } : null,
...
<div class="app-shell">
	<header class="app-header">
		...
		<div class="header-actions">
			...
			><Button variant="outline" onclick={() => (setupOpen = true)} disabled={busy || deleting}
				><Settings2 size={16} /> Setup</Button
			>
		</div>
	</header>
	<main class="workspace">
		<aside class="sidebar">
			...
			<div class="mt-7 flex items-center justify-between">
				<span class="section-label">REVIEWS</span><span class="text-xs text-slate-400"
					>{appState.imports.length}</span
				>
			</div>
						...
						>{#if item.source === 'google'}<Cloud size={18} />{:else}<FolderOpen
							...
							><small
								>{fmt(item.count)} photos · {item.status === 'complete'
									...
									: 'Paused'}</small
							></span
						>{#if importId === item.id}<span class="active-dot"></span>{/if}</button
					>{/each}
			</div>
			{#if !appState.imports.length}<p class="mt-4 text-sm leading-relaxed text-slate-400">
			...
			{#if job}<div class="mt-7 border-t pt-5">
					...
					><label class="check-label mt-3"
						><Checkbox bind:checked={useList} disabled={busy || deleting} /><span
							>Use a filename list</span
						></label
					>{#if useList}<Textarea
			...
			<div class="sidebar-bottom">
				...
				<p class="mt-1 text-xs text-slate-400">{fmt(appState.localCount)} photos indexed</p>
		...
		<section class="review-main">
			...
			{#if notice}<div role="status" class="notice">
					<span>{notice}</span><button aria-label="Dismiss notice" onclick={() => (notice = '')}
						><X size={16} /></button
					>
				</div>{/if}
			{#if unresolved.length}<div class="notice error">
					<span
						>{unresolved.length} deletion attempt(s) need verification. They will not be retried automatically.</span
					><Button size="sm" variant="outline" disabled={busy || deleting} onclick={verifyPending}
						>Check status</Button
					>
				</div>{/if}
			<div class="review-heading">
				<div>
					<div class="section-label">
						{job?.source === 'google' ? 'GOOGLE PHOTOS' : job ? 'LOCAL PHOTOS' : 'PHOTO WORKSPACE'}
					</div>
					<h2>
						{job
							? job.source === 'google'
				...
				<div class="flex items-center gap-2">
					{#if job?.source === 'google' && job.status !== 'complete'}{#if importing}<Button
								variant="outline"
								size="sm"
								onclick={() => (stopRequested = true)}><Pause size={14} /> Pause import</Button
							>{:else}<Button
								variant="outline"
								size="sm"
								disabled={busy || deleting}
								onclick={() => run(importMore)}><RefreshCw size={14} /> Resume import</Button
							>{/if}{/if}{#if job}<Button
							variant="ghost"
							size="sm"
							onclick={() => run(exportDecisions)}
							disabled={busy || deleting}
							><Download size={15} /><span class="hidden sm:inline">Export decisions</span></Button
						>{/if}
				</div>
			</div>
			{#if importing}<div class="mb-4 flex items-center gap-2 text-sm text-indigo-600">
					<LoaderCircle size={16} class="animate-spin" /> Importing… {fmt(job?.count || 0)} photos saved{skippedVideos
			...
			{#if appState.imports.length}<select
					...
					>{#each appState.imports as item}<option value={item.id}
							>{item.source === 'local' ? 'Local photos' : item.query || 'All Google Photos'} · {fmt(
								item.count
							)}</option
						>{/each}</select
				>{/if}
			...
			<div class="photo-stage">
					...
					{#if currentFailed || !imageUrl(current)}<div class="image-error">
							...
							{#if current.source === 'local'}<Button
								...
								>{/if}{#if current.source === 'google'}<Button
									variant="secondary"
									disabled={busy}
									onclick={refreshPreview}><RefreshCw size={15} /> Refresh preview</Button
								><Button
									variant="secondary"
									href={'https://photos.google.com/photo/' + current.mediaKey}
									target="_blank"
									rel="noreferrer">Open in Google Photos <ExternalLink size={15} /></Button
								>{/if}
						</div>{/if}
					<div class="stage-top">
						<span>{fmt(index + 1)} <span class="text-white/40">/ {fmt(queue.length)}</span></span
						><span class="stage-pill"
							>{current.source === 'local' ? 'Local preview' : 'Google preview'}</span
						>
					</div>
					<button
				...
				{:else}<div class="stage-empty">
						{#if busy}<LoaderCircle size={36} class="animate-spin text-indigo-300" />
							<h3>Preparing your review…</h3>{:else if job && counts.all}<div class="empty-check">
								<Check size={30} />
							</div>
							<h3>
								{selectedView === 'unreviewed' ? 'You’re all caught up.' : 'Nothing here yet.'}
							</h3>
							<p>
							...
							<h3>{job ? 'No photos match these filters.' : 'A fresh look at your library.'}</h3>
							<p>
								{job
							...
							<Button class="mt-5" onclick={() => (setupOpen = true)} disabled={busy}
								>Choose photos <ArrowRight size={16} /></Button
							>{/if}
					</div>{/if}
			</div>
			<div class="photo-meta">
				<div>
					...
					<p>
						{current
							...
							<span>· {current.width} × {current.height}</span>{/if}{#if current?.localMatches}<span
							>
								· {current.localMatches === 1 ? 'Backup match' : 'Multiple backup matches'}</span
							>{/if}
					</p>
				</div>
				{#if current?.source === 'google'}<a
					...
					>{/if}{#if current && current.decision !== 'unreviewed'}<Badge variant="outline"
						>{current.decision === 'delete' ? 'Marked for deletion' : current.decision}</Badge
					>{/if}
			</div>
			<div class="decision-bar">
				<Button
					...
					><Trash2 size={18} /><span>Mark for deletion</span><kbd>D</kbd></Button
				><Button
			...
			<div class="review-footer">
				...
				><span class="text-xs text-slate-400 hidden sm:block">← mark · → keep · Space unsure</span
				><span class="text-sm text-slate-500">{fmt(reviewed)} of {fmt(counts.all)} reviewed</span>
			</div>
			<div class="progress-track">
				<div style:width={(counts.all ? (reviewed / counts.all) * 100 : 0) + '%'}></div>
			</div>
			{#if queue.length > 1}<div class="filmstrip" aria-label="Upcoming photos">
					{#each queue.slice(Math.max(0, index - 2), index + 7) as p}<button
							...
							><PhotoImage photo={p} small loading="lazy" />{#if p.decision === 'keep'}<Check
									size={12}
								/>{:else if p.decision === 'delete'}<Trash2 size={12} />{/if}</button
						>{/each}
				</div>{/if}
			{#if counts.delete}<div class="deletion-summary">
					...
					{#if marked.length}<Button
						...
						>{:else}<Button
							variant="outline"
							onclick={() => run(exportDecisions)}
							disabled={busy || deleting}><Download size={16} /> Export decisions</Button
						>{/if}
				</div>{/if}
		</section>
	</main>
...
<Dialog.Root bind:open={setupOpen}
	><Dialog.Content class="sm:max-w-2xl max-h-[90vh] overflow-y-auto"
		><Dialog.Header
			><Dialog.Title class="text-2xl">Start a review</Dialog.Title><Dialog.Description
		...
		<div class="setup-grid">
			<label class="field-label"
				>Photo source<Select.Root type="single" bind:value={source}
					><Select.Trigger class="w-full"
						>{source === 'google' ? 'Google Photos' : 'Local folder'}</Select.Trigger
					><Select.Content
						><Select.Item value="google">Google Photos</Select.Item><Select.Item value="local"
							>Local folder</Select.Item
						></Select.Content
					></Select.Root
				></label
			>
			{#if source === 'google'}<div class="connection-panel">
					<div class="flex items-center justify-between gap-3">
						...
						<Button variant="outline" size="sm" disabled={busy} onclick={() => run(connect)}
							><Cloud size={15} /> Connect</Button
						>
					</div>
					<div class="mt-3">
						<Button
						...
						>
						<p class="mt-2">Unzip the download, then follow the installation instructions below.</p>
					</div>
					{#if accounts.length}<label class="field-label mt-3"
							>Google Photos account<select class="native-select" bind:value={tabValue}
						...
						>{:else}<button
							...
							onclick={() => (helpOpen = true)}>How to install the extension</button
						>{/if}
				</div>
				<label class="field-label"
					>Import from<Select.Root type="single" bind:value={queryMode}
						...
						><Select.Content
							><Select.Item value="query">Google Photos search</Select.Item><Select.Item value="all"
								>All Google Photos</Select.Item
							></Select.Content
						></Select.Root
					></label
				>{#if queryMode === 'query'}<label class="field-label"
						>Search query<Input bind:value={query} placeholder={'"WA"'} /><span class="field-hint"
			...
			{#if source === 'local' || matchOnly}<div class="field-label">
					...
					<div class="connection-panel">
						...
						<div class="mt-3 flex flex-wrap gap-2">
							...
							{#if folder && folderAccess !== 'granted'}<Button
									variant="outline"
									disabled={busy}
									onclick={() => run(() => reconnectFolder(folder!))}>Reconnect folder</Button
								>{/if}
						</div>
						{#if indexing}<p role="status" class="mt-3">
								Indexing… {fmt(indexCount)} photos found.
							</p>
							<Button size="sm" variant="ghost" onclick={() => indexController?.abort()}
								>Cancel indexing</Button
							>{/if}
						{#if !folderSupported}<p class="mt-3 text-amber-700">
								Folder selection needs Chrome or Edge. Google imports and saved reviews can still be
								used in supported browsers.
							</p>{/if}
					</div>
					<span class="field-hint"
						>Select WhatsApp Images inside Pictures/WhatsApp. Includes subfolders. The app requests
						read access only.</span
					>
					{#if allFolders.length > 1}<div class="flex flex-wrap gap-2">
							{#each allFolders.filter((f) => f.id !== folder?.id) as saved}<Button
									...
									variant="outline"
									disabled={busy}
									onclick={() => run(() => reconnectFolder(saved))}>Use {saved.name}</Button
								>{/each}
						</div>{/if}
				</div>{/if}
			<label class="check-label"
				><Checkbox bind:checked={useList} /><span>Limit to a list of filenames</span></label
			>{#if useList}<Textarea
					bind:value={filenamesText}
		...
		<div class="connection-panel mb-4">
			...
			<p>
				Your reviews stay on this device and website. Export a workspace before switching to GitHub
				Pages.
			</p>
			<div class="mt-3 flex flex-wrap gap-2">
				<Button
					...
					size="sm"
					disabled={busy || deleting}
					onclick={() => run(saveWorkspace)}><Download size={15} /> Export workspace</Button
				><label class="text-sm"
		...
		<Dialog.Footer
			><Button variant="outline" disabled={busy} onclick={() => (setupOpen = false)}>Cancel</Button
			><Button disabled={busy || deleting} onclick={() => run(startReview)}
				>{#if busy}<LoaderCircle size={16} class="animate-spin" />{/if}{source === 'local'
					? 'Start local review'
					: 'Import & start review'}<ArrowRight size={16} /></Button
			></Dialog.Footer
		>{#if error}<p role="alert" class="text-sm text-red-600">{error}</p>{/if}</Dialog.Content
	></Dialog.Root
>
	...
	><AlertDialog.Content class="sm:max-w-4xl max-h-[90vh] flex flex-col"
		><AlertDialog.Header
			...
			><AlertDialog.Description
				>Check the images below, then click Delete again to move them to Google Photos’ trash. This
				can also affect synced devices. Your local backup will stay unchanged.</AlertDialog.Description
			></AlertDialog.Header
		>
		{#if preview}<div class="flex items-center justify-between gap-3 text-sm">
				<Badge variant="outline">{preview.photos[0]?.account}</Badge><span class="text-slate-500"
					>{marked.length > 100 ? 'First 100 marked photos · ' : ''}{loadedImages.length} of {preview
						.photos.length} previews loaded</span
				>
			</div>
			<div class="delete-grid">
				{#each preview.photos as p}<div>
						<PhotoImage
							...
							small
							onready={() => {
								if (!loadedImages.includes(p.id)) loadedImages = [...loadedImages, p.id];
							}}
							onfailure={() => {
								if (!failedImages.includes(p.id)) failedImages = [...failedImages, p.id];
							}}
						/><span title={p.filename}>{p.filename}</span>
					</div>{/each}
			</div>{/if}
		{#if failedImages.length}<p class="text-sm text-red-600">
				Some previews could not be loaded. Cancel and check those photos before deleting.
		...
		<AlertDialog.Footer
			><AlertDialog.Cancel disabled={deleting}>Keep reviewing</AlertDialog.Cancel><Button
				variant="destructive"
				disabled={deleting ||
					busy ||
				...
				>{#if deleting}<LoaderCircle size={16} class="animate-spin" />{:else}<Trash2
						size={16}
					/>{/if}Delete {preview?.photos.length || 0} photos</Button
			></AlertDialog.Footer
		></AlertDialog.Content
	></AlertDialog.Root
>
		...
		<div class="space-y-5 text-sm leading-relaxed">
			...
			<div>
				...
				<ol class="list-decimal space-y-2 pl-5">
					<li>
						Download the extension above and unzip it. Keep the extracted folder while the extension
						is installed.
					</li>
					<li>In Chrome, open <code>chrome://extensions</code> and enable Developer mode.</li>
					<li>
					...
					<li>
						...
						>
						and localhost. After updating the extension, reload it in Chrome’s extensions page, reload
						the app, and choose <strong>Setup → Connect</strong>.
					</li>
				</ol>
			</div>
			<p class="rounded-lg bg-amber-50 p-3 text-amber-900">
				The connector uses Google Photos’ undocumented website API. If Google changes it, imports
				pause and uncertain deletions require verification. Never use “empty trash” during this
				review.
			</p>
			<p>
				Local-folder reviews also work without the extension. Use Export decisions to save a
				portable list of your choices.
			</p>
		</div>
		<Dialog.Footer><Button onclick={() => (helpOpen = false)}>Got it</Button></Dialog.Footer
		></Dialog.Content
	></Dialog.Root
>
