import { db, type ReviewDatabase, type LocalFile, type FolderRecord } from './db';
...
export async function scanDirectory(
	...
	async function walk(directory: FileSystemDirectoryHandle, prefix: string) {
		for await (const [name, entry] of directory.entries()) {
			...
			if (!safeRelativePath(path)) throw new Error('The folder contains an unsupported filename.');
			if (entry.kind === 'directory') await walk(entry as FileSystemDirectoryHandle, path + '/');
				...
				files.push({ folderId, path, filename: name, bytes: file.size, mtime: file.lastModified });
...
export async function indexFolderHandle(
	...
	if (reattachId) {
		...
		if (!existing || existing.name !== handle.name)
			throw new Error(
				'Select the original folder named ' + (existing?.name || 'in this review') + '.'
			);
	} else
	...
	const folder = { id, name: handle.name, handle, indexedAt: Date.now(), count: files.length };
...
export async function readPhotoFile(photo: Photo, database: ReviewDatabase = db): Promise<File> {
	if (photo.source !== 'local' || !photo.folderId || !safeRelativePath(photo.localPath))
		throw new Error('No local file is linked to this photo.');
	const folder = await database.folders.get(photo.folderId);
	if (!folder?.handle) throw new Error('Reconnect this photo’s folder in Setup.');
	...
	if (
		...
		throw new Error('This local file has changed. Reindex the folder and start a new review.');
	return file;
}
