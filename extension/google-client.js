  ...
  try {
      ...
      const response = await fetch(url, {
        ...
        body: new URLSearchParams({
          'f.req': JSON.stringify([
            [[method, JSON.stringify(data), null, 'generic']]
          ]),
          at: g.SNlM0e
        }),
    ...
    async function metadata(keys) {
      ...
      if (!Array.isArray(rows))
        throw new Error('Google Photos metadata format changed.')
      return new Map(
        rows.map((row) => [
          row[0],
          { filename: row?.[1]?.[3], bytes: row?.[1]?.[9] || 0 }
        ])
      )
    }
    ...
    function parseRow(row, info) {
      ...
      return {
        ...
        isOwned:
          Array.isArray(row[7]) &&
          !row[7].some((a) => Array.isArray(a) && a.includes(27)),
    ...
    async function detail(key) {
      ...
      if (d?.[0]?.[0] !== key)
        throw new Error('Photo identity could not be verified.')
      return d
    }
    /** @param {any[][]} d */
    ...
    if (action === 'page') {
      ...
      if (
        ...
        (!Array.isArray(result[0]) && result[0] != null)
      )
      ...
      const info = rows.length
        ? await metadata(rows.map((r) => r[0]))
        : new Map()
      const items = rows
        .map((r) => parseRow(r, info.get(r[0])))
        .filter((p) => !p.isVideo)
      ...
      return {
        ...
        items,
        cursor,
        skippedVideos: rows.length - items.length
      }
    }
    if (action === 'refresh') {
    ...
    if (action === 'verify')
      return { account, trashed: isTrashed(await detail(payload.mediaKey)) }
    if (action === 'trash') {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        await pause(800)
        if (isTrashed(await detail(item.mediaKey)))
    ...
    return {
      error: e instanceof Error ? e.message : 'Google Photos connection failed.'
    }
  }
}
