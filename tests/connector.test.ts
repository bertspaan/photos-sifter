...
describe('Chrome companion RPC boundary (mock Google responses)', () => {
  ...
  it('uses the all-photos source including archived items', async () => {
    ...
    expect(
      await googleRequest('page', { account, query: '', cursor: 'page2' })
    ).toMatchObject({
      items: [],
      cursor: null
    })
  ...
  it('refuses a changed account before any network request', async () => {
    mockRpc(() => null)
    expect(await googleRequest('page', { account: 'other' })).toHaveProperty(
      'error'
    )
  ...
  it('refuses a trash mutation if the filename does not match', async () => {
    mockRpc((method) =>
      method === 'VrseUb'
        ? [row()]
        : [[null, [['photo-a', [null, null, null, 'different.jpg']]]]]
    )
  ...
  it('does not retry a mutation whose response was lost', async () => {
    mockRpc((method) => {
      ...
      if (method === 'EWgK9e')
        return [[null, [['photo-a', [null, null, null, 'IMG-WA.jpg']]]]]
  ...
  it('pauses on malformed server responses', async () => {
    ...
    expect(
      await googleRequest('page', { account, query: 'WA' })
    ).toHaveProperty('error')
  })
})
