...
export function parseFilenames(text: string): string[] {
	...
	if (!trimmed) return [];
	let values: unknown = trimmed.startsWith('[') ? JSON.parse(trimmed) : trimmed.split(/\r?\n/);
...
export function validateDeletion(rows: Photo[]): void {
	...
	for (const p of rows)
		if (
			...
			throw new Error('Only owned Google Photos marked for deletion can be moved to trash.');
}
