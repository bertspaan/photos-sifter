...
The companion runs requests inside the signed-in Google Photos tab. Credentials remain in that tab; the app receives sanitized photo metadata. Google previews load directly from Google. The extension accepts app requests only from the configured local origin or published path, in the top-level frame. It has no wildcard permission to connect arbitrary HTTPS websites.
