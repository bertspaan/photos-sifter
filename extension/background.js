...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (!sender.url || sender.frameId !== 0 || !isAllowedAppUrl(sender.url))
    return false
  async function execute(tabId, action, payload) {
    ...
    if (!value)
      throw new Error(
        'No response from Google Photos. Reload the Google Photos tab.'
      )
  ...
  ;(async () => {
    ...
    .catch((e) =>
      sendResponse({ error: e.message || 'Google Photos request failed.' })
    )
  return true
})
