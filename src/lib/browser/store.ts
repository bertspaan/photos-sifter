...
const googlePhotoId = (account: string, key: string) =>
  'google:' + JSON.stringify([account, key])
...
export class ReviewStore {
  ...
  async assertUnlocked(id: string) {
    if (
      ...
      throw new Error(
        'Verify the previous deletion attempt before changing this photo.'
      )
  }
  ...
  async photos(f: Filters) {
    ...
    const files = folderId
      ? await this.db.files.where('folderId').equals(folderId).toArray()
      : []
    ...
    return rows
      ...
      .map((p) => ({
        ...p,
        importId: f.importId,
        localMatches: counts.get(p.filename) || 0
      }))
      .filter(
        (p) => matchesFilters(p, rest) && (!names.size || names.has(p.filename))
      )
  ...
  async unresolved() {
    const rows = await this.db.deletions
      .where('state')
      .anyOf('executing', 'uncertain')
      .toArray()
    ...
    switch (action) {
      ...
      case 'export': {
        ...
        return {
          ...
          photos: (
            ...
            url:
              source === 'google'
                ? 'https://photos.google.com/photo/' + mediaKey
                : undefined
          }))
        }
      }
      case 'local-import':
        return this.write(async () => {
          ...
          if (!folder?.indexedAt)
            throw new Error('Choose and index a photo folder first.')
          const id = crypto.randomUUID(),
            files = await this.db.files
              .where('folderId')
              .equals(folderId)
              .toArray()
          ...
          const photos = files.map((f, i) => {
            ...
            const changed =
              previous &&
              (previous.bytes !== f.bytes || previous.localModified !== f.mtime)
            ...
            return {
              ...
              decision: changed
                ? ('unreviewed' as const)
                : previous?.decision || 'unreviewed',
              revision: (previous?.revision || 0) + (changed ? 1 : 0)
            }
          })
          await this.db.photos.bulkPut(photos)
          await this.db.memberships.bulkPut(
            ids.map((photoId) => ({ importId: id, photoId }))
          )
      ...
      case 'imports/page': {
        ...
        await this.write(async () => {
          ...
          if (v.cursor && v.cursor === v.expectedCursor)
            throw new Error(
              'Google returned the same page twice. Import paused.'
            )
          ...
          await this.db.photos.bulkPut(
            v.items.map((p, i) => {
              ...
              const identityChanged =
                old &&
                (old.filename !== p.filename || old.dedupKey !== p.dedupKey)
              return {
                ...(old || blankPhoto(ids[i])),
                ...p,
                account: v.account,
                source: 'google',
                decision: identityChanged
                  ? 'unreviewed'
                  : old?.decision || 'unreviewed',
                revision: (old?.revision || 0) + (identityChanged ? 1 : 0)
              } as Photo
            })
          )
          await this.db.memberships.bulkPut(
            ids.map((photoId) => ({ importId: v.id, photoId }))
          )
          await this.db.imports.update(v.id, {
            cursor: v.cursor,
            status: v.cursor ? 'paused' : 'complete',
            error: '',
            count: await this.db.memberships
              .where('importId')
              .equals(v.id)
              .count()
          })
      ...
      case 'photo-refresh': {
        const v = z
          .object({ id: idSchema, account: idSchema, photo: importedSchema })
          .parse(body)
        ...
        return this.write(async () => {
          ...
          if (
            ...
            throw new Error(
              'Photo identity changed. Import it again before reviewing.'
            )
      ...
      case 'imports/error': {
        const v = z
          .object({ id: idSchema, error: z.string().max(2000) })
          .parse(body)
        await this.write(async () => {
          const job = await this.db.imports.get(v.id)
      ...
      case 'decision': {
        ...
        return this.write(async () => {
          ...
          if (p.revision !== v.revision)
            throw new Error(
              'This photo changed in another tab. Reload and try again.'
            )
          await this.db.photos.update(v.id, {
            decision: v.decision,
      ...
      case 'undo':
        return this.write(async () => {
          ...
          for (const h of history) {
            ...
            await this.db.photos.update(p.id, {
              decision: h.previous,
              revision: p.revision + 1
            })
      ...
      case 'delete-preview': {
        ...
        if (new Set(ids).size !== ids.length)
          throw new Error('Duplicate photo selection.')
        return this.write(async () => {
      ...
      case 'delete-confirm': {
        ...
        return this.write(async () => {
          ...
          const rows = await Promise.all(
            preview.ids.map((id) => this.getPhoto(id))
          )
          await this.validateSelection(rows)
      ...
      case 'delete-claim': {
        ...
        return this.write(async () => {
          ...
          const expected = preview?.consumed
            ? JSON.parse(preview.fingerprint).find(
                (r: unknown[]) => r[0] === p.id
              )
            : null
          if (
            !expected ||
            JSON.stringify(expected) !== previewFingerprint([p]).slice(1, -1)
          )
            throw new Error(
              'This decision changed after confirmation. Open a new preview.'
            )
          await this.assertUnlocked(p.id)
      ...
      case 'delete-resolve': {
        const v = z
          .object({ batch: idSchema, id: idSchema, trashed: z.boolean() })
          .parse(body)
        return this.write(async () => {
          const item = await this.db.deletions.get([v.batch, v.id])
          if (!item || !['executing', 'uncertain'].includes(item.state))
