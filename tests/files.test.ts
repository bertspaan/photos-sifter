...
describe('read-only folder handling', () => {
	it('indexes relative paths recursively, skipping hidden files and videos', async () => {
		...
		const handle = directory('Photos', [a, directory('Sent', [b]), hidden, image('video.mp4')]);
		expect(await scanDirectory(handle, 'folder')).toEqual([
			{ folderId: 'folder', path: 'IMG-WA.jpg', filename: 'IMG-WA.jpg', bytes: 3, mtime: 123 },
			{ folderId: 'folder', path: 'Sent/other.png', filename: 'other.png', bytes: 3, mtime: 123 }
		]);
	...
	it('can cancel a scan before any file is read', async () => {
		...
		await expect(
			scanDirectory(directory('Photos', [file]), 'folder', () => {}, controller.signal)
		).rejects.toThrow();
		expect(file.getFile).not.toHaveBeenCalled();
	});
	it('rejects paths that could escape the chosen folder', () => {
		for (const path of ['/a.jpg', '../a.jpg', 'Sent/../../a.jpg', 'Sent//a.jpg', 'Sent\\a.jpg', ''])
	...
	it('does not prompt or read when permission is missing', async () => {
		...
		await expect(readPhotoFile(photo, database)).rejects.toThrow('Allow folder access');
	...
	it('opens a selected file with no write/create access and detects replacements', async () => {
		...
		await expect(readPhotoFile({ ...photo, bytes: 999 }, database)).rejects.toThrow('has changed');
	});
	it('preserves the previous index when saving the new index fails', async () => {
		...
		try {
			await db.folders.put({ id: 'old', name: 'Photos', indexedAt: 1, count: 1 });
			await db.files.put({
				folderId: 'old',
				path: 'old.jpg',
				filename: 'old.jpg',
			...
			await expect(
				indexFolderHandle(directory('Photos', [image('new.jpg')]), () => {}, undefined, 'old', db)
